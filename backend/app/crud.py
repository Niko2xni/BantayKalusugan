...
def delete_user(db: Session, user_id: int, admin_id: int):
    ...
    if db_user:
        ...
        db.query(models.PatientVitalSubmission).filter(models.PatientVitalSubmission.patient_id == user_id).delete()
        # Delete the user
        db.delete(db_user)
        db.commit()
...
def create_patient_vital_submission(
    db: Session,
    patient: models.User,
    payload: schemas.PatientVitalSubmissionCreate,
):
    submission = models.PatientVitalSubmission(
        patient_id=patient.id,
        date=payload.date,
        time=payload.time,
    ...
    _create_notification(
        db,
        user_id=patient.id,
        title="Vital submission received",
        body="Your vital signs were submitted and are pending review by health staff.",
        kind="vitals",
    )

    _notify_admin_users(
        db,
        title="New patient vital submission",
        body=(
            f"{_format_user_full_name(patient)} submitted vitals on "
            f"{submission.date} {submission.time}."
        ),
        kind="vitals",
    )
...
def get_patient_vital_submissions(
    db: Session,
    patient_id: int,
    status: str | None = None,
    skip: int = 0,
    ...
    query = db.query(models.PatientVitalSubmission).filter(models.PatientVitalSubmission.patient_id == patient_id)
    if status:
        query = query.filter(models.PatientVitalSubmission.status == status.strip().lower())

    return (
        query.order_by(
            models.PatientVitalSubmission.created_at.desc(),
            models.PatientVitalSubmission.id.desc(),
        )
        .offset(skip)
        .limit(limit)
        .all()
...
def get_admin_vital_submissions(
    db: Session,
    status: str | None = None,
    skip: int = 0,
    limit: int = 200,
):
    query = db.query(models.PatientVitalSubmission)
    if status:
        query = query.filter(models.PatientVitalSubmission.status == status.strip().lower())
...
def review_patient_vital_submission(
    db: Session,
    current_admin: models.User,
    submission_id: int,
    ...
    submission = (
        db.query(models.PatientVitalSubmission)
        .filter(models.PatientVitalSubmission.id == submission_id)
        .first()
    )
    if submission is None:
        raise ValueError("Vital submission not found")

    if submission.status != "pending":
        raise ValueError(f"Vital submission has already been reviewed as {submission.status}")

    review_status = payload.status.strip().lower()
    submission.status = review_status
    submission.admin_notes = payload.admin_notes.strip() if payload.admin_notes else None
    ...
    if review_status == "approved":
        approver = _format_user_full_name(current_admin)
        created_vital = create_vital_sign(
            db,
            schemas.VitalSignCreate(
                ...
                recorded_by=f"Patient Self-Report (approved by {approver})",
    ...
    if review_status == "approved":
        _create_notification(
            db,
            user_id=submission.patient_id,
            title="Vital submission approved",
            body="Your submitted vital signs were approved and added to your records.",
            kind="vitals",
        )
    else:
        _create_notification(
            db,
            user_id=submission.patient_id,
            title="Vital submission rejected",
            body="Your submitted vital signs were reviewed but not approved. Check staff notes for guidance.",
            kind="vitals",
        )
    ...
    create_audit_log(
        db=db,
        admin_id=current_admin.id,
        action="Approved" if review_status == "approved" else "Rejected",
        target_id=submission.id,
        target_type="Patient Vital Submission",
        details=(
            f"{review_status.title()} patient vital submission {submission.id} "
            f"for patient {submission.patient_id}"
        ),
        source_document_url=submission.source_document_url,
    )

    if created_vital is not None:
        db.refresh(created_vital)

    return submission, created_vital
